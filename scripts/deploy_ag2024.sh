...
time ansible-playbook -i ~/galaxyXpand/environments/ag2024/hosts playbook.yml >> ~/install_log.txt
sleep 60
systemctl restart galaxy.target
sleep 30
galaxyctl status >> ~/install_log.txt
time ansible-playbook -i ~/galaxyXpand/environments/ag2024/hosts install_tools.yml >> ~/install_log.txt
date >> ~/install_log.txt
